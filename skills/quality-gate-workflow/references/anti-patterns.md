...
59. PRD 变更后不声明影响级别就继续执行。用户在 Gate 2 中声明 PRD 有变更时，必须调用 `prd-changed --impact <级别>` 声明影响级别，引擎按级别处理下游步骤重置。不声明 = 流程失控。禁止继续执行而不评估影响。
60. Plan 微调修改了可验证项定义。`--plan-tweak` 仅允许修改 Plan 的实现细节（Task 描述、实现顺序、技术选型），禁止修改可验证项定义（item id、验收标准、PRD section 引用）。涉及可验证项变更时必须升级使用 `--prd-changed`。
