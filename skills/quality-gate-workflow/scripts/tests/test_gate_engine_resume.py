...
class TestResume:
    ...
    def test_resume_running_step_reset(self, engine_instance, tmp_path):
        ...
        assert engine_instance.state["steps"]["P0"]["status"] == RUNNING
        rc = engine_instance.resume()
        assert rc == 0
        assert engine_instance.state["steps"]["P0"]["status"] == NOT_STARTED
    ...
    def test_resume_warnings(self, engine_instance, tmp_path):
        ...
        engine_instance.complete("P0", artifacts=["docs/plans/temp.md"])
        # 删除 artifact
        artifact.unlink()
        rc = engine_instance.resume()
        # resume 仍成功（有警告）
